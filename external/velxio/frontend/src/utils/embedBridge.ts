/**
 * Embed bridge — lets a PARENT page (e.g. Wireup's page 04) drive this Velxio
 * instance when it runs inside an iframe. Wireup vendors Velxio as a
 * submodule and embeds it; without this bridge the only way in is the manual
 * file picker, which no parent page can trigger cross-origin.
 *
 ...
 *     parent, which can fold them into its own diagram.json.
 *
...
export function initEmbedBridge(): void {
  ...
  window.addEventListener('message', (event: MessageEvent) => {
    ...
    if (message.type === 'velxio:load-vlx') {
      ...
      importVlxFile(file)
        ...
        .catch((err: unknown) =>
          reply(event, { type: 'velxio:vlx-error', message: err instanceof Error ? err.message : String(err) }),
        );
      return;
    }

    if (message.type === 'velxio:export-vlx') {
      try {
